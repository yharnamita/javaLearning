...
/*
 * EXERCICIO COM MATRIZES PARA JOGO DE CAMPO MINADO
 */
...
public class Exercicio28 {
	...
	public static void main(String[] args) {
		...
		while (!campo.venceu() && !perdeu) {
			...
			int linha = Utility.lerIntervaloInt(sc, "Linha (1 a 5): ", 1, 5) - 1;

			int coluna = Utility.lerIntervaloInt(sc, "Coluna (1 a 5): ", 1, 5) - 1;
		...
		private int contarMinas(int l, int c) {
			...
			for (int i = -1; i <= 1; i++) {
				for (int j = -1; j <= 1; j++) {
					...
					if (nl >= 0 && nl < linhas && nc >= 0 && nc < colunas && tabuleiro[nl][nc] == '*') {
